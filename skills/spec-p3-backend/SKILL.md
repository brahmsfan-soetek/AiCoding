...
- [`lightweight-review-prompt.md`](../../spec-workflow-refs/p3/lightweight-review-prompt.md) — 邏輯類 task 輕量獨立 review prompt（**實驗中、預設關**，G2-02）
- [`hooks/README.md`](../../spec-workflow-refs/p3/hooks/README.md) — 測試唯讀強制 + 成功靜默 hook 範本（可選，抽救自 eap）
...
3. **Session 長度軟提示（G2-11）** — 每完成數個 task 或 session 明顯變長（回應變慢、出錯率升）時,建議 PG `/clear` 後依 progress.md + api_contract / current_schema（SSOT）resume；不設硬性 N、不是 stop gate。
...
4. **檢查 checkpoint（resume 偵測）：**
   ...
   - 若存在 → 讀取進度,向 PG 報告已完成 / wip / todo 的 **後端** task 摘要（前綴 `B*`）,詢問是否從上次繼續
     ...
     - **Resume 錨定 SSOT（G2-10）**：resume 的依據除 progress.md / session_log.md 外,必須重讀 `api_contract.md` 與 `current_schema_{程式編號}.md` 中與待續 task 相關的小節作錨定——不可只基於上次 AI 的產出接著做（防衍生文件取代源頭的語義漂移）
...
6. **Task Loop（每個 task）：**
   ...
   b. **依類型 tag 分支：**
      ...
      **若為 `[validator]`：**
      ...
      - 寫實作 → 跑測試,確認 Green
        - 若發現實作已先寫完才要補測試（順序顛倒）→ 暫時把 method body 換成 `throw new UnsupportedOperationException()` 補走 Red 再恢復,不可直接跳過 Red（AR003 B21 教訓）
      - lint + typecheck
      - `git diff` 自檢測試檔：若 Red→Green 過程中斷言被修改 → 明確告知 PG（test-to-fit 風險）
      - （可選,**實驗中、預設關**）PG 可另開乾淨 session 依 [`lightweight-review-prompt.md`](../../spec-workflow-refs/p3/lightweight-review-prompt.md) 對本 task 做獨立 review（G2-02,退場條件見 05 D9）
...
8. **模型選擇（G2-12）** — 邏輯類 task（`[validator]` / `[processor]`）與長 context 階段（SG1 大量載入、resume）建議用**最強可用模型**執行本 SKILL（§3：模型等級差異 > prompt 技巧,是最重要的單一決策）。
