...
description: SA 規格書消化流程（S0–S4）：DOCX 轉 MD → 規格統計 → 釐清清單 → SA 回覆整合。在 SA 資料夾隔離執行，純文件作業。完成後接 spec-tasking 於專案內產任務清單。觸發於 /spec 或提到規格統計、規格消化、釐清清單、spec digest 等關鍵字。
---

# Spec Digest Flow — SA 規格書消化流程

PG 接到 SA 規格書後，透過 AI 輔助完成從規格解析到最終版規格統計的標準作業流程。本 SKILL 涵蓋 S0–S4，任務清單階段（原 S5）已獨立為 `spec-tasking` SKILL，於專案 repo 內執行以貼合專案既有慣例。
...
完成 S4 後，請切換到**專案 repo 目錄**，改用 `spec-tasking` SKILL 產出前後端與測試任務清單。
...
2. Parse user arguments:
   ...
   - `5` or `step5` → 告知使用者：S5 已獨立為 `spec-tasking` SKILL，請切換到專案 repo 目錄後改用 `/tasking`。
...
- **Next:** 將 `{程式編號}_規格統計_最終版.md` 與 UI 截圖複製到專案 repo，然後切換到專案目錄執行 `spec-tasking` SKILL 產出任務清單。原始 SA 材料（規格書 MD、HTML、截圖等）於此步完成後可移至 `SA document/` 歸檔。
