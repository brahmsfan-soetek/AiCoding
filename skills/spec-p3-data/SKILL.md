...
[STOP] Scope Statement（切入點 7）
      - Deliverable（一句話）
      - 預期動到的範圍（PERMISSION / seed 表清單 + commit 範圍）
      - 明示 out-of-scope（schema / DDL / 其他程式 / cleanup 等）
...
4. **讀 DB schema（來源 = `current_schema_{程式編號}.md`）：**
   ...
   - **[STOP] Scope Statement（動手前必跑）：** 在進入步驟 5 抽權限需求前，AI 用 3 行報告 lock scope：
     - **Deliverable**（一句話）：本次預計產出 `{程式編號}_PERMISSION.sql` + `{程式編號}_SEED.sql` + 對帳結果 + 維護期 hand-off
     - **預期動到的範圍**：列實際清單（如 PERMISSION 涉及的 Menu / Role / MenuRole 等表名 + seed 涉及的業務表名 + commit 範圍的 SQL 檔位置 + `session_log.md`）
     - **明示 out-of-scope**：列「本次不會動」的範圍（如「不改 schema / DDL」「不動非本程式的 PERMISSION 配置」「不清除既有 seed 殘留（由 PG ad hoc cleanup）」「不寫 backend / frontend code」「不動其他程式的 seed」）
     - PG 確認 scope 後才進入步驟 5 抽權限需求
     - **規約**：產出過程若發現需超出 scope（如需動 schema / 改其他程式 PERMISSION / 順手 cleanup） → STOP 回報 PG，PG 決定擴張或縮回；AI 不自行擴張（對齊 Insight 報告 35 wrong_approach + 12 excessive_changes 觀察）
5. **抽權限需求：**
...
| # | 位置 | 作用 | 可否省略 |
|---|------|------|---------|
| SG0 | 步驟 4「讀 schema」末尾 | **Scope Statement**（Deliverable / 預期動到 / out-of-scope，切入點 7）| **不可省略** |
...
8. **Scope-lock 動手前必跑** — 步驟 4「讀 schema」末尾 Scope Statement（Deliverable / 預期動到 / out-of-scope）為 stop gate；產 SQL 過程發現需超出 scope（如需動 schema / 改其他程式 PERMISSION / 順手 cleanup）→ STOP 回報 PG，不自行擴張（對齊切入點 7 / Insight 報告 35 wrong_approach + 12 excessive_changes 觀察）。
...
7. **Scope-lock 動手前必跑** — 步驟 4「讀 schema」末尾為 Scope Statement stop gate（Deliverable / 預期動到 / out-of-scope）；過程發現需超出 scope → STOP 回報。
