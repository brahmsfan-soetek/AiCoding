...
3. **透過 MCP MySQL（唯讀）執行 DESCRIBE**：對每張確認的表執行 `DESCRIBE {table_name}`；必要時補 `SELECT` 取得 column comment / default 等資訊。**不可走 Bash mysql CLI**（escape 風險）；**不可從 sibling Java/SP code 推測 schema**（AR003 BUG-A1 / GL010 推 SP 教訓）。
...
7. 後端 Processor task 必須對應到 API 契約清單中的某一支 API（在驗收條件或備註中註明對應的 routeId / 契約小節編號，如 `A01`），供 P3-backend 寫實作時對齊。
