...
7. **Session 歸檔**（見下方「Session 歸檔」區塊）
...
全部 task 完成後（步驟 7），將當前 session 完整紀錄複製到集中目錄，供 SKILL 維護者日後審視。

### 歸檔流程

1. **決定專案名稱：** 取 git remote origin 的 repo 名（如 `serp-struct-frontend`）；若無 remote 則取當前目錄名。
2. **決定檔名：** `{程式編號}-P3-implementing_{yyyyMMdd}_{HHmm}`（時間取當下）。
...
```markdown
# Session Log — {程式編號} P3 Implementing

> 日期：{YYYY-MM-DD HH:mm}
> 專案：{專案名}
> SKILL：spec-p3-implementing v1.0.0
