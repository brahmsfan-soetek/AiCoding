...
選配參考（不在必讀範圍）：[`commit-hook/README.md`](../../spec-workflow-refs/p3/commit-hook/README.md)（hook 安裝步驟與替代設計）、[`hooks/README.md`](../../spec-workflow-refs/p3/hooks/README.md)（成功靜默 hook 範本,未接線）。
...
**為什麼是「契約對照」不是「契約測試」：** mock fixture 與真實 BE 會漂移——contract test 抓得到 typo,抓不到跨層欄位 rename / camelCase 落差；SG2 靜態對照共讀契約省 token、無 fixture 漂移風險,PG 一眼可審。UI 類寫測試耗時遠高於 PG 手測 1 秒看出問題的速度。
...
3. **Session 長度軟提示** — 「一氣呵成」指不逐批 demo 打斷,不是不換 session：每完成數個 task 或 session 明顯變長（回應變慢、出錯率升）時,建議 PG `/clear` 後依 progress.md + api_contract（SSOT）resume；不設硬性 N、不是 stop gate。
...
3. **讀取任務清單、共讀契約與專案 context：**
   ...
   - 讀取 `{程式編號}_api_contract.md` — `[service]` / `[store-map]` 契約測試對齊基準（每支 API 一個 A## 小節）
     - 若 api_contract 缺失 → 中止流程,告知 PG 需回 `/tasking` 補；**不可從 sibling FE code 推 response shape,也不可直接讀 BE Java/Processor code**
...
4. **檢查 checkpoint：**
   ...
   - 若存在 → 讀取進度,報告前端（`F*`）task 狀態
     - **Resume 錨定 SSOT**：resume 的依據除 progress.md / session_log.md 外,必須重讀 `api_contract.md` 中與待續 task 相關的 A## 小節作錨定——不可只基於上次 AI 的產出接著做
...
5. **[STOP] SG1 — Scope Statement + 確認載入 + hook 安裝：**
   ...
   - **列前端硬守則清單**（來源：target 前端 `CLAUDE.md` + PG memory 相關條目；SKILL 只負責「讀 + 列 + 驗證」機制,**不可預設 / 硬編任何具體規約內容**——規約權威 = target project）：
     - 分兩類報告,每條註明來源（target `CLAUDE.md` 段落 / PG memory 名稱）：
       - **可 grep 驗證類** — 每條附 regex pattern,作為 SG3 自動驗證依據（例：禁 inline style → `style="`）
       - **意圖類** — 不可機械驗證,PG 手測階段審（例：組件式拆分、樣式參考某既有元件）
     - 兩處來源皆無 → **明確警告 PG,不可靜默跳過**：「⚠️ 本 session 將無前端守則防護（SG3 grep 跳過）,建議補 target CLAUDE.md 前端規約,或明示接受再繼續」
     - PG 過了清單（增刪條目、調 pattern）才繼續
   - **檢查 commit-time hook**（每專案一次性設定,非每 session 重談）：偵測 target `.claude/settings.local.json` 是否已註冊
     - **已裝** → 一行報告「✓ commit-time hook 已安裝」
     - **未裝** → 一行詢問：「安裝 commit-time hook？（commit 時自動跑 `npm run typecheck` + `vitest related`,失敗 block 並通知 PG,不自動修;預設裝）」
       - PG 同意（預設）→ 依 [`commit-hook/README.md`](../../spec-workflow-refs/p3/commit-hook/README.md)「安裝」節執行：複製 `<skill-dir>/templates/hooks/typecheck-test-on-commit.ps1` 到 target `.claude/hooks/`,合併 `settings.local.json.tmpl` 的 PreToolUse 區塊到 `.claude/settings.local.json`（gitignored;保留現有 permissions）;package.json 缺 `typecheck` / `type-check` script 時提示 PG 補（hook 會自動只跑 vitest）
       - PG 跳過 → 本 session 不再詢問;替代設計（typecheck-only 等 spike）見同一 README
   - 詢問 PG：起始 task
6. **Task Loop（每個 task）：**
   a. 讀取 task + 類型 tag
   ...
   b. 分支：
      ...
      **`[service]` / `[store-map]` → 契約對照流程（無 mock-based test）：**
      ...
      - **[STOP] SG2 對照表審**：向 PG 展示「實作意圖 ↔ api_contract A##」對照表,PG 一眼確認對齊
        - **不可省略**
        - 不寫 mock-based 契約測試（fixture 與真實 BE 會漂移；contract test 抓 typo OK,跨層 rename / camelCase 漂移擋不下來）
      - 寫實作（無 mock test）
      - lint + typecheck
   ...
   f. **[STOP] SG3：** PG 審閱（繼續 / 回修 / 停止）,可降密度
      - **UI 類 task（`[page]` / `[dialog]` 等視覺輸出類）SG3 報告加守則 grep 驗證**：
...
| # | 位置 | 作用 | 可否省略 |
|---|------|------|---------|
| SG1 | session 啟動後 | **Scope Statement**（見 [`scope-statement.md`](../../spec-workflow-refs/p3/scope-statement.md)）+ 確認載入正確、類型分佈、**前端硬守則清單**（分可 grep / 意圖兩類）、commit-time hook 狀態（一行）、起始 task | 不建議 |
...
1. **SG2 「實作意圖 ↔ api_contract A##」對照表審** — `[service]` / `[store-map]` 寫實作前必先列對照表,PG 過了才寫實作。`[store-map]` 對照表必須涵蓋 rename / null / 多餘欄位三類欄位處理。**不寫 mock-based 契約測試**（fixture 與真實 BE 會漂移,抓 typo 抓不到跨層 rename）。
...
5. **SG1 前端硬守則清單 + SG3 自動 grep 守則驗證** — SG1 對齊意圖（列守則）、SG3 自動驗證落地（grep 違規清單,**只報不擋**）,攔截「連跑多元件後一次驗收 → 全部偏」的擴散。與 commit-time hook 互補：grep 屬風格層由 PG 判斷,hook 屬 typecheck / test 層自動擋。**規約內容由 target CLAUDE.md / PG memory 提供,SKILL 只負責「讀 + 列 + 驗證」機制,不預設任何具體規約**；target 沒有可 grep 規約 → 兩段都跳過,不增加干擾。
...
7. **模型分層** — P2 已把判斷寫進清單與契約,本 SKILL 預設 **Sonnet 級模型即可執行**（契約層有 SG2 對照擋,UI 層由 PG 手測擋）；PG 視 task 複雜度自行升級。
