...
| 7 | Scope-lock prompt pattern：restate deliverable + out-of-scope 後才動手 | 全 skill 開頭 + ad hoc 請求 | 直擊 Insight 報告的 35 wrong_approach + 12 excessive_changes 友擦 | ✅ **本輪已實作**（4 SKILL 加 Scope Statement step：spec-p2-tasking 步驟 5 末尾 / spec-p3-backend SG1 第一子段 / spec-p3-frontend SG1 第一子段 / spec-p3-data 步驟 4 末尾 SG0）|
...
## 已實作：切入點 7 — Scope-lock prompt pattern（2026-05-22）

### 落地範圍

只動 spec-workflow SKILL（對齊切入點 9 撤回啟示：ad hoc / target CLAUDE.md / 個人工作風格不歸 SKILL）：

| SKILL | 落點 | 階段意義 |
|---|---|---|
| `spec-p2-tasking` | 步驟 5「與使用者確認」末尾（DB schema 建立前） | 確認程式編號 / 輸出位置後同步 lock scope，避免 task 清單擴張到非規格範圍 |
| `spec-p3-backend` | SG1 第一子段（在「報告載入清單」之前） | session 啟動立即 lock scope，task loop 內每步可回頭對 |
| `spec-p3-frontend` | SG1 第一子段（在「報告載入清單」之前） | 同 backend；特別擋 dialog 過度修改 / sibling 元件順手重構 |
| `spec-p3-data` | 步驟 4「讀 schema」末尾 SG0（產 SQL 前） | 讀完 schema 後 lock scope，避免擴張到非本程式的 PERMISSION / 順手 cleanup |

不動 `spec-p1-digest-flow`（S0–S4 流程固定，每階段 scope 已內定，restate 重複度高）。
不寫 target project CLAUDE.md / 個人 global CLAUDE.md（對齊切入點 9 撤回原則：target / 個人事 SKILL 不該管）。

### 設計範本（4 SKILL 共用結構，僅變換上下文措辭）

每個 SKILL 的 Scope Statement 段固定 3 行 + 1 條規約：

1. **Deliverable**（一句話）：本次預計產出什麼
2. **預期動到的範圍**：列實際清單（檔名 / 表名 / commit 範圍，不只大類）
3. **明示 out-of-scope**：列「本次不會動」的範圍（依 SKILL 上下文填入：sibling 程式 / 契約 / schema / DDL / 全域 i18n / 重構 / 其他 task / 跨層 code 等）

**規約**：實作中若發現需超出 scope → STOP 回報 PG，PG 決定擴張或縮回；AI 不自行擴張。

### Insight Report 原始範本（為比對留檔）
...
落地時改寫為中文 + SKILL 上下文化（如 spec-p3-backend 列「不改 `current_schema` / `api_contract`」、spec-p3-frontend 列「不改全域 i18n / 順手調 dialog width」、spec-p3-data 列「不動 schema / 不清 seed 殘留」），不直接 verbatim 引用英文 prompt。

### 改造方案

| 改 | 目的 | 動到的檔案 |
|:--:|---|---|
| 改 1：spec-p2-tasking 步驟 5 末尾加 Scope Statement 子段 | 進入 schema 建立 / task 產出前 lock scope | `spec-p2-tasking/SKILL.md` 步驟 5 / 核心原則 11 |
| 改 2：spec-p3-backend SG1 加 Scope Statement 為第一子段 | session 啟動立即 lock scope | `spec-p3-backend/SKILL.md` SG1 / Execution Flow box / Stop Gate 表 / 防護機制 10 |
| 改 3：spec-p3-frontend SG1 加 Scope Statement 為第一子段 | 同 backend；針對 UI 擴散風險加 dialog width / sibling 元件等 out-of-scope 範例 | `spec-p3-frontend/SKILL.md` SG1 / Execution Flow box / Stop Gate 表 / 防護機制 8 |
| 改 4：spec-p3-data 步驟 4 末尾加 Scope Statement（SG0） | 讀完 schema 後 lock scope，新增獨立 SG0 stop gate | `spec-p3-data/SKILL.md` 步驟 4 / Execution Flow box / Stop Gate 表（加 SG0 row）/ 防護機制 8 / 核心原則 7 |
| 改 5：4 SKILL 對應 README 同步 | 流程總覽 / Stop Gate 表 / 核心原則 一致 | `spec-p2-tasking/README.md`、`spec-p3-backend/README.md`、`spec-p3-frontend/README.md`、`spec-p3-data/README.md` |

### 影響檔案清單

| 檔案 | 改動類型 |
|---|---|
| `skills/spec-p2-tasking/SKILL.md` | 步驟 5 末尾加 Scope Statement 子段；核心原則加第 11 條（Scope-lock） |
| `skills/spec-p2-tasking/README.md` | 核心原則加第 8 條；流程總覽圖在「PG 確認」box 加 Scope Statement 三行 |
| `skills/spec-p3-backend/SKILL.md` | 步驟 5（SG1）開頭加 Scope Statement 子段；Stop Gate 表 SG1 描述；Execution Flow box SG1 行；關鍵防護機制加第 10 條 |
| `skills/spec-p3-backend/README.md` | 核心原則加第 6 條；Stop Gate 表 SG1 描述；流程總覽 SG1 行 |
| `skills/spec-p3-frontend/SKILL.md` | 步驟 5（SG1）開頭加 Scope Statement 子段；Stop Gate 表 SG1 描述；Execution Flow box SG1 行；關鍵防護機制加第 8 條 |
| `skills/spec-p3-frontend/README.md` | 核心原則加第 7 條；Stop Gate 表 SG1 描述；流程總覽 SG1 行 |
| `skills/spec-p3-data/SKILL.md` | 步驟 4 末尾加 Scope Statement 子段；Execution Flow box 加 Scope Statement step；Stop Gate 表加 SG0 row；關鍵防護機制加第 8 條；核心原則加第 7 條 |
| `skills/spec-p3-data/README.md` | 核心原則加第 8 條；Stop Gate 表加 SG0 row；流程總覽加 Scope Statement step |

### 預期擋下的事件（基於 log + Insight 反推）

| 事件 | 改 1 (p2) | 改 2 (p3-be) | 改 3 (p3-fe) | 改 4 (p3-data) |
|---|:---:|:---:|:---:|:---:|
| AR002Q spec 被加未要求的 export feature | ✓ | — | — | — |
| AR002 dialog width 過度修改 | — | — | ✓ | — |
| AR002 / AR003「順手重構」相鄰程式碼 | — | ✓ | ✓ | — |
| AR003 F20-F27 連跑 8 元件後一次驗收 → 全部偏（scope 漂） | — | — | ✓ | — |
| 順手改 `api_contract` / `current_schema`（跨層污染） | — | ✓ | ✓ | — |
| spec-p3-data 順手 cleanup 其他程式 seed 殘留 | — | — | — | ✓ |
| Insight 35 wrong_approach + 12 excessive_changes 大宗 | ✓ | ✓ | ✓ | ✓ |

### 設計取捨

- **不加到每個 task loop 內 restate**：太冗，違反 SKILL 整體精簡原則；scope 鎖在 SKILL 整體入口（SG1 / 步驟 4），task loop 內若需擴張 → 走「實作中發現需超出 scope → STOP 回報」規約
- **不寫進 target CLAUDE.md**：對齊切入點 9 撤回啟示，target CLAUDE.md 屬 target project 不歸 spec-workflow SKILL；PG 若想加全域 ad hoc Scope-lock 規約 → 個人 global `~/.claude/CLAUDE.md`
- **bug 修復走 ad hoc**：切入點 5 已立規約「bug 修復不入 SKILL」，所以 bug 修復場的 Scope-lock 也不歸 SKILL，由 PG ad hoc 自管

### 後續驗證建議

1. 在 target project（serp / eap）跑任一 SKILL，看 AI 是否真的在 SG1 / 對應步驟報告 Scope Statement 三行（Deliverable / 預期動到 / out-of-scope）
2. 故意在實作中讓 AI 「順手」碰非 scope 檔案，看 AI 是否真的 STOP 回報而非自行擴張
3. 觀察 Insight Report 下批統計 wrong_approach + excessive_changes 數字是否下降（基準：本批 35 + 12）
4. 若驗證通過 → 收口；若 AI 仍頻繁順手擴張 → 補強 prompt（如改成「列 out-of-scope 不足 N 項則拒絕進下步」這類硬性條款）
