...
- UI 截圖：每張圖的頁面名稱、欄位佈局、按鈕位置、互動元素
- DDL/CSV：每張表的欄位名、型態、長度、nullable、預設值、鍵別
- 邏輯文件：覆寫或釐清的條目

### Step 2：規格書 ↔ UI 截圖比對

**使用 Read 工具讀取每張 UI 截圖 PNG，辨識畫面內容。**

| 比對維度 | 常見衝突 |
|---------|---------|
| 欄位名稱與數量 | 規格書列 5 個欄位，截圖顯示 7 個 |
| 狀態碼與列舉值 | 規格書寫 A/P，截圖顯示 DRAFT/POSTED/CANCELLED |
| 按鈕與操作流程 | 規格書寫「刪除」，截圖無對應按鈕 |
| 頁面跳轉邏輯 | 規格書的 Use Case 流程 vs 截圖的實際連結 |
| 必填/選填標記 | 規格書標為選填，截圖有 `*` 必填標記 |
| **UI 互動模式** | **規格書寫「跳轉頁面」，但截圖實際呈現為 Dialog 彈窗（或反之）** |

> **⚠️ UI 互動模式判斷**：規格書常用「頁面」一詞描述所有子功能畫面（如 detail、detail2），但這**不代表**前端必須建立獨立路由頁面。必須觀察 UI 截圖的實際呈現：
> - 截圖以彈窗/overlay/modal 呈現 → **Dialog 模式**
> - 截圖以完整獨立頁面呈現（有獨立標題、返回導航） → **Page 模式**
> - eap 專案預設使用 Dialog 模式
...
| **DDL typo** | **如 `VACATION_SUN_ID` 實為 `VACATION_SUB_ID`，需比對上下文確認** |
...
4. 假別只有一個細項時的處理方式（自動帶入，不顯示下拉）

### Step 4：UI 截圖 ↔ DDL/CSV 比對

| 比對維度 | 常見衝突 |
|---------|---------|
| 欄位對應 | 截圖顯示的欄位在 DDL 中不存在 |
| 資料型態 | 截圖顯示為文字輸入，DDL 為 INT/BIT |
| 下拉選項 | 截圖的選項與 DDL 備註的 LOOKUP 不一致 |
...
>
> ## 級聯下拉邏輯
> - 前端使用統一 LOV 呼叫取得假別+假別細項完整清單
> - 假別下拉：從 LOV 結果按 vacationCode 去重
> - 假別細項下拉：按選中的 vacationCode 篩選
> - 只有一個細項時：自動帶入，不顯示下拉（顯示文字）
> - 多個細項時：顯示下拉讓使用者選擇
> ```
...
- **DDL 欄位名可能有 typo** — 如 `VACATION_SUN_ID` 實為 `VACATION_SUB_ID`，需從上下文確認
- 規格書中的「頁面」≠ 前端的獨立路由頁面 — 必須以 UI 截圖的實際呈現方式為準
- **FK 關係是前端下拉邏輯的根源** — 遺漏 FK 分析 = 前端下拉必定出錯
- **DDL 中某些記錄有雙重身份** — 如 `VACATION_SUB_CODE IS NULL` 的記錄既是「沒有細項的假別」，也是有效的 `VACATION_SUB_ID` 值
- UI 截圖中展示用的假資料不代表業務規則 — 以規格書和邏輯文件為準
