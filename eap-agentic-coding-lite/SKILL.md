...
> **⚠️ 前端 UI 互動模式（Dialog vs Page）**：eap 專案預設使用 **Dialog 模式**（新增/編輯/批次用 SDialog2 彈窗，不為 CRUD 建獨立路由頁面）。僅在明細頁面極為複雜時（如 PM001 有 12+ 區塊）才使用 Page 模式。此決策在 P0 的統一規格中明確標註，並在 P1 的任務拆解中強制執行。參見 `conventions/code-patterns-frontend.md`「❌ 為 CRUD 建獨立路由（用 Dialog）」。
...
| [conventions/code-patterns-frontend.md](conventions/code-patterns-frontend.md) | 前端代碼模式 + 禁止模式 | **P1（UI 模式決策）**、P2（前端 Task） |
...
| 主頁面 `{MODULE_CODE}.vue` | [templates/frontend-page.md](templates/frontend-page.md) | **僅用於主頁面**（含 Dialog 整合） |
| Dialog 組件 `*Dialog.vue` | [templates/frontend-dialog.md](templates/frontend-dialog.md) | Dialog 模式下的 Create/Edit/Batch |

> **⚠️ 模板選擇**：`frontend-page.md` 用於主頁面（清單查詢），不用於 Create/Edit/Batch 組件。Dialog 模式下的 Create/Edit/Batch 必須使用 `frontend-dialog.md`。
...
- **規格書的「頁面」≠ 前端的路由頁面** — SA 文件習慣把每個子功能都稱為「頁面」（如「修改頁面」「新增頁面」），但前端預設使用 Dialog 彈窗，不建立獨立路由。必須在 P0 統一規格中明確標註 UI 互動模式（Dialog / Page），並在 P1 強制執行。
