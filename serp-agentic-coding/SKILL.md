...
專案：serp_struct（Quarkus 3.25.4 + Vue 3 + Quasar 2.14 + MySQL）。

**核心原則**：AI 做執行，人做判斷。每次 LLM 處理都是有損轉換 — 結構性防禦優於指令性約束。

## When to Use

**適用**：serp 的規格驅動功能開發（DOCX + HTML 輸入）
...
serp 的規範拆為獨立檔案，各 Phase 按需載入相關的規範，不一次全載 (§2 + §10)。
...
| `*{CODE}.vue`（頁面） | [templates/frontend-page.md](templates/frontend-page.md) |
| `*ListSection.vue` | [templates/frontend-list-section.md](templates/frontend-list-section.md) |
