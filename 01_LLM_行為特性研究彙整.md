...
> ⚠️ 審查報告指出的歸屬錯誤與數據修正尚未套用，見 `review/2026-03-23-r1-審查報告.md`
...
> 來源：[LLM as a Broken Telephone (ACL 2025)](https://aclanthology.org/2025.acl-long.371/)；[When LLMs Play the Telephone Game (Acerbi et al. 2024)](https://arxiv.org/abs/2407.04503)
...
Context Rot 在多輪對話中有更具體的量化表現。對 200,000+ 模擬對話、15 個模型的實測顯示，將同一任務分散到多輪對話，平均效能下降 **39%**（Laban et al., ICLR 2025）。
...
| 發現 | 數值 | 來源 |
|------|------|------|
| Context 長度本身降低效能（即使資訊完全相關） | **13.9%-85%** 下降 | Du et al., EMNLP 2025 |
| 插入 25,000 個空白字元（零資訊量）仍導致錯誤答案 | 顯著退化 | Du et al., EMNLP 2025 |
| 多文件摘要中的幻覺內容比例 | 最高 **75%** | NAACL 2025 |
| 被要求摘要不存在主題時仍生成摘要 | GPT-3.5: **79%** | NAACL 2025 |
| 語義相關但不相關的噪訊 vs 完全無關噪訊 | 前者**更具干擾性** | arXiv:2505.21870 |
| 策劃過的 context vs 未策劃 context | 減少 **60% 錯誤** | Inkeep 2025 |
...
> 來源：[Context Length Alone Hurts LLM Performance (Du et al., EMNLP 2025)](https://arxiv.org/abs/2510.05381)；[Multi-Doc Summarization Hallucination (NAACL 2025)](https://arxiv.org/abs/2410.13961)；[Semantic Noise in RAG (arXiv:2505.21870)](https://arxiv.org/html/2505.21870v1)；[Inkeep: Context Engineering (2025)](https://inkeep.com/blog/context-engineering-why-agents-fail)；[Same Task More Tokens (2024)](https://arxiv.org/abs/2402.14848)
...
| 模型對 context vs parametric memory 的混合比例 | ~70% context / ~30% parametric | arXiv:2409.08435, 2024 |
| ChatGPT 輸出中含自相矛盾的句子比例 | **17.7%** | Mündler et al., ICLR 2024 |
| 其中無法由外部文本驗證的矛盾（純內部混合） | **35.2%** | Mündler et al., ICLR 2024 |
| RAG 場景最佳 baseline 在矛盾文件上的 exact match | **32.60** | MADAM-RAG, 2025 |
...
| Meta 工程師接受 LLM 測試 vs 實際相關 | 73% 接受，僅 **36%** 實際相關 | Meta, FSE 2025 |
| 自動生成測試對真實 bug 的偵測率（單一工具最高） | **19.9%** | Shamshiri et al., ASE 2015 |
...
> 來源：[LLM Test Oracle: Actual vs Expected (Konstantinou et al. 2024)](https://arxiv.org/abs/2410.21136)；[Coverage vs Mutation Score (Cuze & Wotawa, 2025)](https://link.springer.com/chapter/10.1007/978-3-032-05188-2_16)；[Mutation-Guided Test at Meta (FSE 2025)](https://arxiv.org/abs/2501.12862)；[Specification Mining Practicality (ICPC 2019)](https://arxiv.org/abs/1903.03542)；[Auto Test Bug Detection (ASE 2015)](https://homes.cs.washington.edu/~rjust/publ/unit_test_generation_effectiveness_ase_2015.pdf)
...
**三種退化曲線模式**：
- **推理模型**（o1、DeepSeek-R1）：在 ~100-250 條前維持高遵循率，之後斷崖下降——推理能力提供了「緩衝區」，但有上限
- **標準模型**（GPT-4o、Claude 3.5 Sonnet）：從一開始就線性退化——每增加一條指令，遵循率穩定下降
- **小模型**（7B-13B）：指數崩潰——少量指令就嚴重退化
...
| 發現 | 數值 | 來源 |
|------|------|------|
| Python pass@1：<10 行 vs >50 行 | **0.872 → 0.265**（70% 下降） | NeurIPS 2023 |
...
| 同一模型（GPT-4）單次 vs 迭代 Agent（SWE-bench） | **1.96% → 12.47%**（6.4× 提升） | SWE-bench, ICLR 2024 |
...
> 來源：[Code Generation Quality vs Length (NeurIPS 2023)](https://proceedings.neurips.cc/paper_files/paper/2023/file/43e9d647ccd3e4b7b5baab53f0368686-Paper-Conference.pdf)；[CodeChain (ICLR 2024)](https://proceedings.iclr.cc/paper_files/paper/2024/file/6111371a868af8dcfba0f96ad9e25ae3-Paper-Conference.pdf)；[Self-Collaboration (ACM TOSEM 2024)](https://dl.acm.org/doi/10.1145/3672459)；[SWE-bench (ICLR 2024)](https://www.swebench.com/original.html)；[FeatureBench (ICLR 2026)](https://arxiv.org/abs/2602.10975)
...
| AI 代碼未通過安全測試 | 45%；漏洞為人類 **2.74×**；XSS **86%**、Log Injection **88%** | [Veracode 2025 GenAI Report](https://www.veracode.com/blog/genai-code-security-report/) | §5 頻率偏見（訓練資料中的不安全模式） |
| Amazon AI 事故（90 天 4 個 Sev-1） | AWS Cost Explorer 13hr 中斷、99% 訂單下降 | [CNBC 2026/03](https://www.cnbc.com/2026/03/10/amazon-plans-deep-dive-internal-meeting-address-ai-related-outages.html) | 未驗證即合併的系統性風險 |
...
| Context 過載 | 長度本身降低效能 13.9-85%；策劃 context 減少 60% 錯誤；推理在 ~3K tokens 退化 | Du et al. EMNLP 2025; Inkeep 2025; Same Task 2024 |
...
| 任務複雜度天花板 | pass@1 <10 行 0.872 vs >50 行 0.265；分解 +35-76%；迭代 vs 單次 6.4× | NeurIPS 2023; ICLR 2024; ICLR 2026 |
| 測試生成模式複製 | LLM oracle 準確率 <50%；93.5% 覆蓋率但 58.8% mutation score；Meta 36% 相關 | Konstantinou 2024; Cuze & Wotawa 2025; Meta FSE 2025 |
...
| When Context Leads but Parametric Memory Follows (2024) | https://arxiv.org/abs/2409.08435 |
...
| Semantic Noise in RAG (arXiv:2505.21870) | https://arxiv.org/html/2505.21870v1 |
...
| Is Your Code Generated by ChatGPT Really Correct? (NeurIPS 2023) | https://proceedings.neurips.cc/paper_files/paper/2023/file/43e9d647ccd3e4b7b5baab53f0368686-Paper-Conference.pdf |
...
- [x] §1 語義漂移的衰減率 — 原數字無研究支持，已移除。改引用 ACL 2025 和 Acerbi 2024 的定性描述
