...
名義上的 context window ≠ 有效 context。以下整理主流模型的 context window 容量與退化閾值估計。

> **⚠️ 退化閾值欄的說明**：目前沒有研究對每個模型逐一實測「從第幾 K token 開始品質加速下降」的退化曲線。表中的退化閾值是根據以下有限數據點推估的，不是實測值：
> 1. Chroma（2025）泛指「200K 窗口的模型在 50K 就出現顯著退化」——以此作為 200K 級模型的基準
> 2. Chroma（2025）泛指「所有模型在累積 80K–150K tokens 後普遍退化」——以此作為 agentic session 的通用上限
> 3. MRCR v2 分數（Opus 4.6: 76%、Sonnet 4.5: 18.5%、Gemini 3.1 Pro: 128K 時 84.9% → 1M 時 26.3%）——以此推斷同為 1M 窗口的模型，MRCR 高分者閾值較高、低分者閾值較低。Gemini 3 系列在 128K→1M 之間存在斷崖式下降，因此其退化閾值設在 80K–128K
>
> 128K 窗口模型按 Chroma 的 200K 基準等比例下修；缺乏任何 benchmark 數據的模型標為「待實測」。**這些數字僅供粗略參考，實際退化點取決於任務類型、信噪比與 context 組成。**

**主流模型 Context Window 一覽（2026 Q1）：**

| 廠商 | 模型 | 名義 Context Window | 退化閾值（超過即加速下降） | 備註 |
|------|------|---------------------|--------------------------|------|
...
| Google | **Gemini 3.1 Pro** | **1,048K** | 80K–128K | MRCR v2 @ 128K: **84.9%**，@ 1M: **26.3%**（128K→1M 斷崖式下降） |
| Google | **Gemini 3 Flash** | **1,048K** | 80K–128K | 比 2.5 Pro 快 3 倍，token 用量少 30% |
| Google | Gemini 3 Pro | 1,048K | 80K–128K | MRCR v2 @ 128K: 77%，@ 1M: 26.3%（已被 3.1 Pro 取代） |
| Google | Gemini 2.5 Pro | 1,048K | 100K–200K | Recall: 530K 時 100%，1M 時 99.7%（舊世代） |
| Google | Gemini 2.5 Flash | 1,048K | 80K–150K | 快速低成本（舊世代） |
