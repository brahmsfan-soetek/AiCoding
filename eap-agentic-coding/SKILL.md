...
**適用**：eap 的規格驅動功能開發（規格書 + HTML 畫面 + DDL/CSV + 邏輯文件）
...
| 規格書路徑 | SA 文件 MD 或 DOCX（Phase 0 主要輸入） | `specs/TM-員工年度假別額度維護.md` |
...
**輸入**: 規格書(MD/DOCX), HTML 畫面目錄, DDL/CSV 目錄, 邏輯文件目錄
...
Phase 2 啟動時建議註冊 `PreToolUse` hook，結構性阻擋 Code Agent 修改測試檔案：
...
> **為什麼用 hook 而非指令？** §9：反作弊指令幾乎無效（o3 被告知不要作弊後仍 14/20 次博弈）。Hook 是確定性控制流 (03-P6)，Agent 無法繞過。Phase 2 結束後移除 hook 即可。
