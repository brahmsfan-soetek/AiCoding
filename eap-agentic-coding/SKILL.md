...
| `*Dialog.vue` | [templates/frontend-dialog.md](templates/frontend-dialog.md)（新增/編輯 Dialog） |
...
確認後：
1. 存入 `.agentic/config.json`（記錄當前模組代號）
2. 建立模組目錄 `.agentic/{moduleCode}/`

每個模組的 Phase 產出物存放在各自的模組目錄下，不同模組互不干擾：

```
{projectRoot}/
  .agentic/
    config.json          ← Setup 建立（當前模組指標）
    .session-stats       ← statusline 自動寫入（session 級，跨模組共用）
    {moduleCode}/        ← 模組專屬目錄（如 TM002/, TM003/）
      conflicts.md       ← P0 產出
      sa_pending.md      ← P0 產出
      ...
      phase-log.md       ← Phase 日誌
      .timers/           ← Phase 計時器（自動管理）
...
> 若 `.agentic/config.json` 已存在，顯示當前配置。使用者可確認沿用現有模組、或指定新模組代號建立新的模組目錄。(02-P5)

### Hooks 前置設定（一次性）

以下 hooks 跨模組共用，首次使用本 Skill 時自動設定，後續模組不需重複。

**檢查邏輯**（Setup 階段執行）：

1. 檢查 eap 專案的 `.claude/hooks/` 是否已有以下 4 個腳本：
   - `statusline.sh` — statusline 自動擷取 token/費用
   - `phase-logger.sh` — Phase 計時與 token 追蹤
   - `block-test-edit.sh` — Phase 2 測試檔案唯讀強制
   - `silent-test-pass.sh` — 測試輸出靜默（成功靜默，失敗詳述）

2. 缺少的腳本 → 從本 Skill 的 `.claude/hooks/` 目錄讀取內容，寫入 eap 專案的 `.claude/hooks/`

3. 檢查 eap 專案的 `.claude/settings.local.json` 是否已包含 hooks 配置。缺少 → 寫入或合併：
...
> **為什麼在 Setup 而非 Phase 2 設定？** Hooks 不分模組、不分 Phase，是整個 Skill 的基礎設施。Setup 時設一次，所有後續模組和 Phase 直接使用。

### 後端建置前置條件（一次性）

eap 是 Maven 多模組專案，`application` 模組的測試依賴其他子模組的 artifact。Phase 2 執行測試前，所有子模組必須已經 install 到本地 Maven repo。

**檢查邏輯**（Setup 階段執行）：

提醒使用者在開始 Phase 2 前先執行：

```bash
cd backend && mvn clean install -DskipTests
```

> 此步驟只需在首次或 pom.xml 有變動後執行。若跳過，Phase 2 的 Code Agent 會因為找不到依賴而反覆嘗試編譯，浪費大量 token。

---

## Phase 日誌

每個 Phase 進場/出場時自動記錄時間戳、token 耗用、費用，產出 `.agentic/{moduleCode}/phase-log.md`。

### 運作原理

```
statusline.sh ──(每次互動)──→ .agentic/.session-stats  (JSON snapshot，session 級)
                                       ↑
phase-logger.sh start Px ──→ 記錄開始時間 + snapshot 基準 → .agentic/{moduleCode}/.timers/
phase-logger.sh end   Px ──→ 讀取最新 snapshot，算差值 → 寫入 .agentic/{moduleCode}/phase-log.md
```
...
`phase-logger.sh` 自動從 `.agentic/config.json` 讀取 `moduleCode`，將 phase-log.md 和 .timers/ 寫入對應的模組目錄。
...
> Statusline 和 hooks 在 Setup 階段已配置完成（見 Setup > Hooks 前置設定），此處直接使用。
...
> 計時器和 snapshot 存於 `.agentic/{moduleCode}/.timers/`（自動管理）。累計耗時從第一個 Phase 的 start 開始計算。同一 session 跨 Phase 時自動算差值（只記該 Phase 的增量）。
...
所有 hook 腳本的模板存放在本 Skill 的 `.claude/hooks/` 目錄，**跨模組共用**。Setup 階段自動安裝到 eap 專案（見 Setup > Hooks 前置設定），後續模組不需重複。

### Hook 清單

| 腳本 | 類型 | 功能 | 生效時機 |
|------|------|------|---------|
| `statusline.sh` | statusLine | 每次互動後擷取 token/費用，寫入 `.agentic/.session-stats` | 全 Phase |
| `phase-logger.sh` | 手動呼叫 | Phase 進場/出場計時，產出 phase-log.md | 全 Phase |
| `block-test-edit.sh` | PreToolUse (Edit\|Write) | 攔截對測試檔案的修改 | 全 Phase（P2 為主要防禦對象） |
| `silent-test-pass.sh` | PostToolUse (Bash) | 測試通過時靜默輸出，失敗時完整呈現 | 全 Phase |

### 設計原則

- **block-test-edit.sh**：結構性阻擋 Code Agent 修改測試檔案。exit 2 = Claude Code 阻止操作並注入提示訊息。比 prompt 指令「不要改測試」可靠 43× (§9：30.4% vs 0.7%)
- **silent-test-pass.sh**：成功是靜默的（不佔 context），失敗才呈現完整輸出。4000 行通過的測試湧入 context → agent 幻覺 (03-P7)
- **常駐不移除**：hooks 安裝後持續生效，不在 Phase 結束時移除。非 P2 Phase 中觸發 block-test-edit 不影響正常操作（只攔截測試檔案）
