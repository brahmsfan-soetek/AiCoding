...
**v0.5 新增**：§8.1-8.5 反模式全面補充實證來源（+16 篇研究）
...
**實證**：
- Context 長度本身就會降低品質——即使模型能完美檢索所有相關資訊，效能仍下降 **13.9%-85%**；插入 25,000 個空白字元（零資訊量）也會導致錯誤答案（[Du et al., EMNLP 2025](https://arxiv.org/abs/2510.05381)）
- 多文件摘要中最高 **75% 的內容是幻覺**；被要求摘要不存在的主題時，GPT-3.5 仍在 79% 的情況下生成摘要（[NAACL 2025](https://arxiv.org/abs/2410.13961)）
- 語義相關但不相關的噪訊比完全無關的噪訊**更具干擾性**——同一專案的規格書、HTML、會議紀錄正是這種最大干擾源（[arXiv:2505.21870](https://arxiv.org/html/2505.21870v1)）
- 策劃過的 context 可減少 **60% 錯誤**（[Inkeep 2025](https://inkeep.com/blog/context-engineering-why-agents-fail)）
...
**實證**：
- 20 個前沿模型中最佳者在 500 條指令時僅達 **68% 遵循率**；三種退化曲線——推理模型在 ~100-250 條後斷崖下降，標準模型線性退化，小模型指數崩潰（[IFScale, 2025](https://arxiv.org/abs/2507.11538)）
- 10 條同時指令的全部遵守率：GPT-4o **15%**，Claude 3.5 Sonnet **44%**——遵循率呈指數衰減 `P(全遵守) = P(單條)^N`（[ManyIFEval, 2024](https://openreview.net/forum?id=R6q67CDBCH)）
- 工具選擇準確率：所有工具描述塞入 prompt 時 **13.62%**，RAG 篩選後 **43.13%**——減少 prompt 體積直接提升 3 倍（[RAG-MCP, 2025](https://arxiv.org/abs/2505.03275)）
- 推理效能在 **~3,000 tokens** 就開始退化，遠低於技術上限（[Same Task More Tokens, 2024](https://arxiv.org/abs/2402.14848)）
...
**實證**：
- Python pass@1 隨代碼長度從 **0.872**（<10 行）降至 **0.265**（>50 行）——70% 的相對下降（[NeurIPS 2023](https://proceedings.neurips.cc/paper_files/paper/2023/file/43e9d647ccd3e4b7b5baab53f0368686-Paper-Conference.pdf)）
- 分解生成 vs 單次生成的改善：APPS **+35%**、CodeContests **+76%**（[CodeChain, ICLR 2024](https://proceedings.iclr.cc/paper_files/paper/2024/file/6111371a868af8dcfba0f96ad9e25ae3-Paper-Conference.pdf)）；三角色分工 **+29.9%-47.1%**（[Self-Collaboration, ACM TOSEM 2024](https://dl.acm.org/doi/10.1145/3672459)）
- SWE-bench：單次 RAG patch 生成 **1.96%** 解決率 vs 迭代 Agent **12.47%**——同一模型（GPT-4）僅因切換為迭代方式就提升 **6.4 倍**（[SWE-bench, ICLR 2024](https://www.swebench.com/original.html)）
- 複雜多檔案任務（FeatureBench）：即使 SWE-bench 74.4% 的模型也只解決 **11.0%**——確認大型任務需要分解（[FeatureBench, ICLR 2026](https://arxiv.org/abs/2602.10975)）
...
**實證**：
- **Amazon 事件（2025/12-2026/03）**：AI 輔助代碼變更導致 90 天內 **4 個 Sev-1 事故**——包括 AWS Cost Explorer 13 小時中斷、北美市場 99% 訂單下降（估計損失 630 萬筆訂單）。內部文件描述「Gen-AI assisted changes」具有「high blast radius」，現已要求資深工程師簽核（[CNBC 2026/03](https://www.cnbc.com/2026/03/10/amazon-plans-deep-dive-internal-meeting-address-ai-related-outages.html)）
- **自動化偏見**：使用 AI 的開發者寫出**更不安全的代碼**，卻**更相信自己的代碼是安全的**（47 名開發者 RCT，[Stanford/Boneh, CCS 2023](https://arxiv.org/abs/2211.03622)）
- **59%** 的開發者使用自己不完全理解的 AI 代碼（[Clutch 2025](https://clutch.co/resources/devs-use-ai-generated-code-they-dont-understand)）；僅 **10%** 掃描大部分 AI 代碼（[Snyk 2023](https://go.snyk.io/2023-ai-code-security-report.html)）
- AI 代碼含安全漏洞 **2.74 倍**於人類代碼；XSS 漏洞率 **86%**、Log Injection **88%**（[Veracode 2025](https://www.veracode.com/blog/genai-code-security-report/)）
- 每 25% AI 採用率增加：交付穩定性下降 **7.2%**（[Google DORA 2024](https://cloud.google.com/blog/products/devops-sre/announcing-the-2024-dora-report)）
...
**實證**：
- **LLM 生成的 test oracle 捕捉的是實際行為而非預期行為**——給定有 bug 的代碼，LLM 的 oracle 準確率 **<50%**，確認 LLM 跟隨實作而非規格（[Konstantinou et al., 2024](https://arxiv.org/abs/2410.21136)）
- **覆蓋率 ≠ 缺陷偵測**：LLM 生成測試達 93.5% 覆蓋率但僅 **58.8% mutation score**（35 百分點差距）；極端案例 100% 覆蓋率但僅 **4% mutation score**——執行每一行但遺漏 96% 的潛在缺陷（[Cuze & Wotawa, 2025](https://link.springer.com/chapter/10.1007/978-3-032-05188-2_16)）
- 自動化規格挖掘的**誤判率 90-99%**——從代碼反推規格在實務上不可行（[Mashhadi & Hemmati, ICPC 2019](https://arxiv.org/abs/1903.03542)）
- 自動生成測試在 357 個真實 bug 中，單一工具最多偵測 **19.9%**（[Shamshiri et al., ASE 2015](https://homes.cs.washington.edu/~rjust/publ/unit_test_generation_effectiveness_ase_2015.pdf)）
- Meta 實踐：工程師接受 73% 的 LLM 生成測試，但僅 **36% 實際相關**——64% 的接受測試未解決真正的關注點（[Meta, FSE 2025](https://arxiv.org/abs/2501.12862)）
...
| Context Length Alone Hurts LLM Performance (Du et al., EMNLP 2025) | https://arxiv.org/abs/2510.05381 |
| From Single to Multi: LLM Hallucination in Multi-Doc Summarization (NAACL 2025) | https://arxiv.org/abs/2410.13961 |
...
| Is Your Code Generated by ChatGPT Really Correct? (NeurIPS 2023) | https://proceedings.neurips.cc/paper_files/paper/2023/ |
| CodeChain: Towards Modular Code Generation (ICLR 2024) | https://openreview.net/forum?id=vSHMSBEIku |
...
| Do Users Write More Insecure Code with AI Assistants? (CCS 2023) | https://arxiv.org/abs/2211.03622 |
...
| Specification Mining Practicality (Mashhadi & Hemmati, ICPC 2019) | https://arxiv.org/abs/1903.03542 |
...
- [x] §8.1 Raw Dump — 補充 4 篇實證（EMNLP 2025、NAACL 2025、語義噪訊研究、Inkeep）
- [x] §8.2 Over-Prompting — 補充 4 篇實證（IFScale 500 條 68%、ManyIFEval 指數衰減、RAG-MCP 3× 改善、推理退化 ~3K tokens）
- [x] §8.3 One-Shot Everything — 補充 4 篇實證（NeurIPS 2023 70% 下降、CodeChain +76%、SWE-bench 6.4×、FeatureBench 11%）
- [x] §8.4 No Verification — 補充 5 篇實證（Amazon 4 Sev-1、Stanford 自動化偏見、Clutch 59%、Veracode 2.74×、DORA -7.2%）
- [x] §8.5 Downstream Backfill — 補充 5 篇實證（Oracle <50%、Mutation 58.8%、規格挖掘 90-99% 誤判、ASE 2015 19.9%、Meta 36%）
- [x] §10 新增 16 篇參考來源
