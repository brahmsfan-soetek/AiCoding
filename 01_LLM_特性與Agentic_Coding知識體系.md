...
### 新增章節（genuinely distinct）

- [ ] **Sycophantic Bias（附和偏見）** — RLHF 訓練導致模型壓制異議附和使用者，即使使用者的方案有誤（[Sycophancy in LLMs, 2024](https://arxiv.org/abs/2411.15287)）
- [ ] **Self-Correction Blind Spot（自我修正盲區）** — 64.5% 盲區率，無法審核自己的產出（[Self-Correction Bench, 2025](https://arxiv.org/abs/2507.02778)）
- [ ] **Reward Hacking（規格博弈）** — Agent 鑽測試漏洞而非解決問題，GPT-5 在 76% 的不可能任務上繞過測試（[ImpossibleBench, 2025](https://arxiv.org/html/2510.20270v1)；[METR 2025/06](https://metr.org/blog/2025-06-05-recent-reward-hacking/)）
- [ ] **Action Perseveration（退化迴圈）** — 無狀態決策導致重試死循環，是 Agentic 環境特有的失敗模式（[Anthropic: Effective Harnesses](https://www.anthropic.com/engineering/effective-harnesses-for-long-running-agents)）

### 併入既有段落

- [ ] Multi-Turn Degradation → 併入 §3.2 Context Rot（[LLMs Get Lost in Multi-Turn, ICLR 2025](https://openreview.net/pdf?id=VKGTGGcwl6)）
- [ ] Premature Action Without Grounding → 併入 §3.6 幻覺
- [ ] Package Hallucination / Slopsquatting → 併入 §3.6 幻覺（[USENIX Security 2025](https://arxiv.org/abs/2406.10279)）
