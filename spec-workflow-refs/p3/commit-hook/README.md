# commit-time hook — typecheck + test on commit

> **適用：** P3-be / P3-fe（P3-data 無 hook，純 SQL 一次性產出）
> **對應切入點：** 8（review/2026-04-09 direction.md A path）
> **範圍：** Windows + PowerShell；Maven（後端）/ npm + vitest（前端）。其他平台請另行翻譯。

當 AI 跑 `git commit` 時，自動跑改動 module / staged FE 檔的 typecheck + test；失敗 → block commit 並要求 AI 通知 PG（不自動修）。
...
1. **AI flow 不被切碎**：原設計每 Edit 卡 5-20s（vue-tsc 慢、mvn compile 也慢），連續修小東西（如 UI 微調加 `class="xxx"`）連環卡；新設計只在 commit 時卡（commit 本來就慢，等 30-60s 可忍受）
...
4. **SG3 已是天然 typecheck stop gate**：每個 task 結束 SG3 報告本來就要報 typecheck 結果，hook 補位「AI 跳過跑 typecheck 直接報全 green」這個極端
...
是 `git commit` →
     ↓
git diff --cached --name-only | grep <平台特定副檔名>
     ↓
無 staged 相關檔 → exit 0
有 staged 相關檔 →
     ↓
...
- 讀 package.json scripts：優先 `typecheck`，退而 `type-check`
- 跑 `npm run -s <typecheck-script>`
- typecheck 通過後，若 `node_modules\.bin\vitest` 存在，跑 `npx --no-install vitest related --run <staged files>`
- `vitest related --run <files>` 是 vitest 內建的「找與這些檔案相依的 test」mode — 不跑全 test suite，只跑可能受影響的 test

## block 後 AI 行為

Reason 明示「**⚠️ Do NOT auto-fix in-place**」，要求 AI 收到 block 後：

1. 看 reason 找出失敗 module / 檔案 + 第一行錯誤
2. 一句話猜測 root cause
...
2. 合併本目錄的 `settings.local.json.tmpl` 的 hooks 區塊到 target project 的 `.claude/settings.local.json`（保留現有 `permissions` 等其他區塊）
3. 確認 target project：
   - P3-be：pom.xml 結構正確（reactor root + modules 結構標準）
   - P3-fe：package.json 有 `typecheck` 或 `type-check` script
...
- **vue-tsc 在大型專案首次跑 30s+**：incremental cache 可降到 10-15s；首次 PG 用 SKILL 前先跑一次 `npm run typecheck` 暖 cache
...
- **跨 module 改動**（Maven）：staged 涵蓋 module A + B → hook 跑 `mvn -pl A,B -am test`，若 B 依賴 A、A 的 test 失敗，B 不會被測；可接受（A 修完 commit 重跑會 cascade）
...
- **切入點 3**（測試策略瘦身）拔了 Processor mock test、store-map / service contract test，剩 `[validator]` 純函式邊界 test。本 hook 對應 `mvn test` / `vitest related` 跑的就是這些剩下保留的測試 — 不再 token 浪費跑 mock test，但保留的 test 強制跑。P3-fe 若 target project 完全沒寫 vitest test（所有測試走手測），hook 的 test 段自動 skip，僅跑 typecheck — 也是合理 fallback
...
如果跑 test 太慢，可改為 typecheck only：
- P3-be：把 script 最後的 `mvn -pl ... -am test` 改成 `mvn -pl ... compile`，跳過 test
- P3-fe：移除 script 內 vitest 段

失去「test 自動跑」的核心價值，但 typecheck 紀律仍在。

### Spike 2（typecheck + lint on commit）

- P3-be：在 mvn compile 後加 `mvn -pl ... checkstyle:check`。需 target project pom.xml 已配置 maven-checkstyle-plugin
- P3-fe：在 typecheck 通過後加 `npm run -s lint`（若 package.json 有）
